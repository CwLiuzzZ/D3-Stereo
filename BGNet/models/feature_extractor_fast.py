...
#         self.firstconv = convbn_relu(1, 32, 7, 2, 3, 1)
        self.firstconv = nn.Sequential(convbn_relu(1, 32, 3, 2, 1, 1), # 3,2; 7,2; 11,2 # SF:11 SL:2
                               convbn_relu(32, 32, 3, 1, 1, 1), 
                               convbn_relu(32, 32, 3, 1, 1, 1))
        self.layer1 = self._make_layer(BasicBlock, 32, 1, 1, 1, 1) #  15,2; 19,2  # SF:19 SL:2
        self.layer2 = self._make_layer(BasicBlock, 64, 1, 2, 1, 1) # SF:31 SL:4
        self.layer3 = self._make_layer(BasicBlock, 128, 1, 2, 1, 1) # SF:55 SL:8
        self.layer4 = self._make_layer(BasicBlock, 128, 1, 1, 1, 1) # SF:87 SL:8
...
#         x = self.deconv4b(x, rem3)
    ...
    def DDFM(self,x):
        ...
        for i in range(len(self.firstconv)):
            ...
            # if i == 2:
                # feature_list.append(x) # 1/2 and 6
        # x = self.firstconv(x) # 1/2
        _,x = self.layer1[0](x,True) # 1/2
        # feature_list.append(_) # 1/2 and 6
        # x = self.layer1[0](x) # 1/2
        # conv0a = x
        feature_list.append(x) # 1/2 and 6
        # x = self.layer2(x) # 1/4
        _,x = self.layer2[0](x,True) # 1/4
        # feature_list.append(_) # 1/4 and 8
        feature_list.append(x) # 1/4 and 8
        x = self.layer3(x) # 1/8
        feat0 = x
        x = self.layer4(x) # 1/8
        feat1 = x
        ...
        feat2 = x
        rem0 = x
        x = self.conv1a(x) # 1/16
        rem1 = x
        x = self.conv2a(x) # 1/32
        rem2 = x
        x = self.conv3a(x) # 1/8
        rem3 = x

        x = self.deconv3a(x, rem2) # 1/32
        rem2 = x
        x = self.deconv2a(x, rem1) # 1/16
        rem1 = x
        x = self.deconv1a(x, rem0) # 1/8
        feat3 = x
        rem0 = x
        #1/2
        x = self.conv1b(x, rem1) # 1/16
        rem1 = x
        x = self.conv2b(x, rem2) # 1/32
        rem2 = x
        x = self.conv3b(x, rem3) # 1/64
        rem3 = x
        #1/16
#         x = self.conv4b(x, rem4)

#         x = self.deconv4b(x, rem3)
        x = self.deconv3b(x, rem2) # 1/32
        x = self.deconv2b(x, rem1) # 1/16
        x = self.deconv1b(x, rem0) # 1/8
        feat4 = x
        gwc_feature = torch.cat((feat0,feat1,feat2,feat3,feat4),dim = 1) # 1/8 and 12
        feature_list.append(gwc_feature)
        return feature_list