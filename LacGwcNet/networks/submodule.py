...
class feature_extraction(nn.Module):
    def __init__(self, structure_fc, fuse_mode, affinity_settings):
        ...
        self.firstconv = nn.Sequential(convbn(3, 32, 3, 2, 1, 1), # SF:11 SL:2
                                       nn.ReLU(inplace=True),
        ...
        self.layer1 = self._make_layer(BasicBlock, 32, 3, 1, 1, 1) # SF:35 SL:2
    ...
    def forward(self, x):
        output = self.firstconv(x) # 1/2
        output_l1 = self.layer1(output) # 1/2
        output_l2 = self.layer2(output_l1) # 1/4
        output_l3 = self.layer3(output_l2) # 1/4
        output_l4 = self.layer4(output_l3) # 1/4
        ...
        if self.sfc > 0: # 4
            ...
            elif self.fuse_mode == 'separate': # default
                # embedding_l1 = self.embedding_l1(output_l1)
                # l1_sf, l1_affi = self.to_sf_l1(embedding_l1)

                embedding_l2 = self.embedding_l2(output_l2.detach()) # 1/4
                l2_sf, l2_affi = self.to_sf_l2(embedding_l2) # 1/4,1/4

                embedding_l3 = self.embedding_l3(output_l3.detach()) # 1/4
                l3_sf, l3_affi = self.to_sf_l3(embedding_l3) # 1/4,1/4

                embedding_l4 = self.embedding_l4(output_l4.detach()) # 1/4
                l4_sf, l4_affi = self.to_sf_l4(embedding_l4) # 1/4

                # output_feature = self.lastconv(torch.cat((l2_sf, l3_sf, l4_sf), dim=1))
                output_feature = self.lastconv(torch.cat((cat_feature, l2_sf, l3_sf, l4_sf), dim=1)) # 1/4
    ...
    def DDFM(self, x):
        ...
        output_l1 = self.layer1(output) # 1/2
        feature_list.append(output_l1)
        output_l2 = self.layer2(output_l1) # 1/4
        output_l3 = self.layer3(output_l2) # 1/4
        output_l4 = self.layer4(output_l3) # 1/4
        feature_list.append(output_l4)
