...
class UniMatch(nn.Module):
    ...
    def forward(self, img0, img1,
        ...
        feature0_list, feature1_list = self.extract_feature(img0, img1)  # list of features # [1/8 and 1/4]

        if seed_pyramid:
            results_dict.update({'left_feature': feature0_list[-1]})
            results_dict.update({'right_feature': feature1_list[-1]})
            return results_dict
        ...
        for scale_idx in range(self.num_scales): # range(2)
            ...
            results_dict.update({'left_feature': feature0_ori})
            results_dict.update({'right_feature': feature1_ori})
    ...
    def DDFM(self,x):
        features = self.backbone.DDFM(x)  # list of [2B, C, H, W], resolution from high to low
        return features